...
class MerchantDashboardSerializer(serializers.ModelSerializer):
    ...
    def get__business_branches(self, obj):
        ...
        return [
            {
                "unique_id": branch.unique_id, "branchName": branch.branchName, 
                "phone":branch.phone, "email": branch.email,  "county": branch.county , 
                "cityTown": branch.cityTown , "physicalAddress": branch.physicalAddress,
                "isPrimary": branch.isPrimary, "isActive": branch.isActive
                } for branch in branches
            ]
    
    def get__gross_net_payout(self, obj):
        return obj.payout_history.filter(status='COMPLETED').aggregate(total_payout=Sum('net_payout_amount'))['total_payout'] or 0.00  
  
  
    def get__gross_sales_today(self, obj):
      yesterday = timezone.now() - timedelta(hours=24)
      result = obj.payout_history.filter(processed_at__gte=yesterday).aggregate(total_sum=Sum('net_payout_amount'))
      return result['total_sum'] or 0.00
  
    def get__awaiting_orders_queue(self, obj):
        return obj.incoming_orders.filter(status = "AWAITING_ALLOCATION").count()
    
    def get__catalog_stock_items_overview(self,obj):
        products =  obj.products.all()
        return [{"unique_id":product.unique_id, "title":product.title, "category":product.categoryPersist, "currentStock":product.stockQuantity , "minimumStockThreshhold":product.minimumStockThreshhold,   "price":product.dealPrice } for product in products ][:20]
    
    def get__catalog_low_stock_items(self,obj):
        low_stock_items = obj.products.filter(stockAuantity__lte =  4)
        return [{"unique_id":product.unique_id, "title":product.title, "category":product.categoryPersist, "currentStock":product.stockQuantity , "minimumStockThreshhold":product.minimumStockThreshhold,   "price":product.dealPrice } for product in low_stock_items ][:20]
    
    def get__merchant_alerts(self,obj):
        recent_alerts = obj.merchant_alerts.filter(Read=False).order_by('-created_at')
        return [
            {
                "Type": alert.Type,
    ...
    def get__incoming_orders(self,obj):
        recent_orders = obj.incoming_orders.filter(status='AWAITING_ALLOCATION').order_by('-created_at')[:5]
        return [
            {
                "unique_id": order.unique_id,
                "order_id": order.order_id,
                "shippingCustomerName": order.shippingCustomerName,
                "branch": order.fulfillmentBranch.branchName,
...
class MerchantInventoryStockSerializer(serializers.ModelSerializer):
    available_to_sell = serializers.ReadOnlyField()
    reserved_quantity = serializers.IntegerField(read_only=True)  # System managed, locked from manual editing

    class Meta:
        model = MerchantInventoryStock
        fields = '__all__'

    def validate(self, data):
        # Safe baseline checks
        quantity = data.get('quantity_in_stock', 0)
        # On updates, extract from instance if not sent
        if self.instance:
            reserved = self.instance.reserved_quantity
        else:
            reserved = 0
            
        if reserved > quantity:
            raise serializers.ValidationError("Physical warehouse stock cannot be updated below currently reserved allocations.")
        return data
