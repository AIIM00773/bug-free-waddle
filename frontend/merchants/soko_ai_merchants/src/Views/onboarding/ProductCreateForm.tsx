...
import { 
  ...
  Loader2 // Added Loader2 for the submission spinning state
} from 'lucide-react';
import { useInventory } from "../../Providers/InventoryProvider";
import { v4 as uuidv4 } from 'uuid';

export function ProductCreateForm({ active_inventory, onCancel, isOpen = true }) {
  const [currentStep, setCurrentStep] = useState(1);
  const [error, setError] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false); // New UI loading state
  const totalSteps = 4;
  
  const [mediaFiles, setMediaFiles] = useState([]); 
  const [mediaPreviews, setMediaPreviews] = useState([]); 

  // Destructured the error state from your context as well
  const { onboardNewItem, productOnboardingSuccess, productOnboardingError } = useInventory();
   
  const [formInputs, setFormInputs] = useState({
    parrentBranch : active_inventory?.parrentBranch ||'', 
  ...
  // Automatically close modal and reset when onboarding is successful
  useEffect(() => {
    if (productOnboardingSuccess) {
      onCancel(); 
      // You could also trigger a global Toast notification here!
    }
  }, [productOnboardingSuccess, onCancel]);

  useEffect(() => {
    if (active_inventory?.unique_id) {
      setFormInputs(prev => ({ ...prev, parrentInventory: active_inventory.unique_id }));
    }
  }, [active_inventory]);
  ...
  useEffect(() => {
    if (!formInputs.category || !formInputs.title) return;

    const catCode = formInputs.category
      .trim()
      .toUpperCase()
      .replace(/[^A-Z0-9]/g, '')
      .slice(0, 3);

    const words = formInputs.title.trim().toUpperCase().split(/\s+/);
    let titleCode = words
    ...
    if (titleCode.length < 3) {
      titleCode = formInputs.title.trim().toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 3);
    }
  ...
  }, [formInputs.category, formInputs.title]);
  ...
  const validateStep = () => {
    ...
    if (currentStep === 2) {
      if (!formInputs.originalPrice || !formInputs.dealPrice) {
        setError("Pricing values are required configuration configurations.");
        return false;
      }
      if (Number(formInputs.dealPrice) > Number(formInputs.originalPrice)) {
  ...
  const handleNext = () => {
    ...
    if (currentStep === 2 && !formInputs.isPhysical) {
      setCurrentStep(4);
    } else {
  ...
  const handleBack = () => {
    setError('');
    if (currentStep === 4 && !formInputs.isPhysical) {
      setCurrentStep(2);
    } else {
  ...
  // Upgraded Submit Handler with Loading State
  const handleSubmit = async (e) => {
    if (e) e.preventDefault();
    if (!validateStep()) return;
    
    setIsSubmitting(true);
    await onboardNewItem(formInputs); 
  ...
  // Combine local validation errors with backend context errors
  const displayError = error || productOnboardingError;
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 antialiased">
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl border border-slate-100 flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300" role="dialog" aria-modal="true">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
          ...
          <button onClick={onCancel} disabled={isSubmitting} className="p-1 rounded-md text-slate-400 hover:text-slate-600 hover:bg-slate-50 transition-colors md:hidden disabled:opacity-50">
            <X className="h-5 w-5" />
          </button>
        </div>
              ...
              <section className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4 animate-in fade-in duration-200">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="col-span-2">
                    ...
                    <input required type="text" name="title" value={formInputs.title} onChange={handleChange} className={inputClass} placeholder="e.g., Sony WH-1000XM4 Wireless Headphones" />
                  </div>
                  <div>
                    <label className={labelClass}>Active Working Category <span className="text-red-500">*</span></label>
                    <input required type="text" name="category" value={formInputs.category} onChange={handleChange} className={inputClass} placeholder="e.g., Electronics, Fitness, Apparel" />
                  </div>
                  <div>
                    <label className={labelClass}>Brand <span className="text-slate-400 font-normal">(Optional)</span></label>
                    <input type="text" name="brand" value={formInputs.brand} onChange={handleChange} className={inputClass} placeholder="e.g., Sony (Leave blank for generic)" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input type="text" name="sku" disabled value={formInputs.sku || 'Awaiting Title & Category...'} className={`${inputClass} font-mono text-xs text-slate-500 tracking-wider`} />
                  </div>
                  <div className="col-span-2">
                    <label className={labelClass}>Description Summary</label>
                    <textarea name="description" rows={3} value={formInputs.description} onChange={handleChange} className={`${inputClass} resize-none`} placeholder="Describe core merchant value metrics..." />
                  </div>
                </div>
              </section>
            )}
              ...
              <section className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-5 animate-in fade-in duration-200">
                <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                  ...
                  <div className="flex gap-4">
                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      <input type="checkbox" name="isTaxExempt" checked={formInputs.isTaxExempt} onChange={handleChange} className="h-4 w-4 rounded border-slate-300 text-slate-950 focus:ring-slate-950 accent-slate-950" />
                      <span className="text-xs font-semibold text-slate-700">Tax Exempt</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      <input type="checkbox" name="isPhysical" checked={formInputs.isPhysical} onChange={handleChange} className="h-4 w-4 rounded border-slate-300 text-slate-950 focus:ring-slate-950 accent-slate-950" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className="md:col-span-2">
                    ...
                    <input required type="number" min="0" step="0.01" name="originalPrice" value={formInputs.originalPrice} onChange={handleChange} className={inputClass} placeholder="0.00" />
                  </div>
                  <div className="md:col-span-2">
                    <label className={labelClass}>Active Deal Markdown (KES) <span className="text-red-500">*</span></label>
                    <input required type="number" min="0" step="0.01" name="dealPrice" value={formInputs.dealPrice} onChange={handleChange} className={inputClass} placeholder="0.00" />
                  </div>
                  <div>
                    <label className={labelClass}>Initial Stock Count</label>
                    <input type="number" min="0" name="stockQuantity" value={formInputs.stockQuantity} onChange={handleChange} className={inputClass} />
                  </div>
                  <div>
                    <label className={labelClass}>Low Stock Cap</label>
                    <input type="number" min="0" name="minimumStockThreshold" value={formInputs.minimumStockThreshold} onChange={handleChange} className={inputClass} />
                  </div>
                  <div className="md:col-span-2">
                    <label className={labelClass}>Physical Warehouse Location Tag</label>
                    <input type="text" name="shelfLocationSnapshot" value={formInputs.shelfLocationSnapshot} onChange={handleChange} className={inputClass} placeholder="Aisle 3, Row B" />
                  </div>
                </div>
              </section>
            )}
              ...
              <section className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4 animate-in fade-in duration-200">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <input type="number" step="0.01" name="weightKg" value={formInputs.weightKg} onChange={handleChange} className={inputClass} placeholder="0.0" />
                  </div>
                  <div>
                    <label className={labelClass}>Length (cm)</label>
                    <input type="number" name="length" value={formInputs.length} onChange={handleChange} className={inputClass} placeholder="0" />
                  </div>
                  <div>
                    <label className={labelClass}>Width (cm)</label>
                    <input type="number" name="width" value={formInputs.width} onChange={handleChange} className={inputClass} placeholder="0" />
                  </div>
                  <div>
                    <label className={labelClass}>Height (cm)</label>
                    <input type="number" name="height" value={formInputs.height} onChange={handleChange} className={inputClass} placeholder="0" />
                  </div>
                  <div className="col-span-2">
                    <label className={labelClass}>Color Profiles (Comma separated)</label>
                    <input type="text" name="color" value={formInputs.color} onChange={handleChange} className={inputClass} placeholder="Black, Space Gray" />
                  </div>
                  <div className="col-span-2">
                    <label className={labelClass}>Sizes Configured</label>
                    <input type="text" name="size" value={formInputs.size} onChange={handleChange} className={inputClass} placeholder="XL, L, 42, 40" />
                  </div>
                </div>
              </section>
            )}
            ...
            {currentStep === 4 && (
              <section className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-5 animate-in fade-in duration-200">
                <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                  ...
                  {mediaFiles.length > 0 && !isSubmitting && (
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-slate-50 border-t border-slate-100 shrink-0">
          <div>
            {currentStep > 1 ? (
              <button type="button" onClick={handleBack} disabled={isSubmitting} className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-600 hover:text-slate-950 transition-colors uppercase tracking-wider disabled:opacity-50">
                <ChevronLeft className="h-4 w-4" /> Previous Step
              </button>
            ) : (
              <button type="button" onClick={onCancel} disabled={isSubmitting} className="inline-flex items-center gap-1.5 text-xs font-bold text-red-500 hover:text-red-700 transition-colors uppercase tracking-wider disabled:opacity-50">
                Cancel & Exit
              </button>
            )}
          </div>
          ...
          <div>
            {currentStep < totalSteps ? (
              ...
              <button 
                ...
                className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-emerald-600 px-6 py-2.5 text-xs font-bold text-white shadow-md hover:bg-emerald-700 transition-all uppercase tracking-wider disabled:opacity-70 min-w-[160px]"
              >
                {isSubmitting ? (
                  <>
