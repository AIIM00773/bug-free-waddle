...
export function MpesaModal({ onSuccessPayment }) {
  // Consume unified logistics, layout, and error states directly from context
  const {
    cartSummary,
    mpesaPhone,
    setMpesaPhone,
  ...
  const billTotal = cartSummary.finalTotal || 0;
  const handleClose = () => {setShowCheckoutModal(false);};
  
  const executeSTKPush = () => {
    setMpesaStatus('sending');
    setTimeout(() => {setMpesaStatus('pin_prompted');}, 2000);
  };
  ...
  const confirmSimulatedPayment = () => {
    ...
    setTimeout(() => {
      ...
      // Construct transaction message payload for the customer message stream
      const mockTx = `SGR${Math.floor(100000 + Math.random() * 900000)}`;
      
      const paymentMsg = {
        id: Date.now().toString(),
        sender: 'ai',
        text: `M-Pesa payment of KES ${billTotal} processed! TXID: ${mockTx}. Your designated runner is carrying your supplies to the door of: ${deliveryNote}. Expected eta is 12 mins.`
      };
  ...
  return (
    <div className="fixed inset-0 bg-zinc-950/85 backdrop-blur-xs flex items-center justify-center p-4 z-50">
      <div className="bg-zinc-950 text-white border border-zinc-800/80 rounded-2xl w-full max-w-sm p-6 shadow-2xl relative overflow-hidden">
        ...
        {/* Modal Close Button */}
        <button onClick={handleClose} className="absolute top-4 right-4 text-zinc-500 hover:text-zinc-300 transition-colors">
          <X size={18} />
        </button>
        ...
        <div className="text-center mt-2">
          ...
          <div className="w-14 h-14 bg-emerald-500/10 text-emerald-400 rounded-full flex items-center justify-center mx-auto mb-3.5 border border-emerald-500/25">
            <Smartphone size={28} className={mpesaStatus === 'sending' ? 'animate-bounce' : 'animate-pulse'} />
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800/80 rounded-xl p-3.5 my-5 text-left space-y-2">
            ...
            <div className="flex justify-between text-xs items-center">
              ...
              <div className="text-right">
                <input 
                  ...
                  className="font-mono font-bold bg-transparent text-right outline-none focus:text-emerald-400 max-w-[140px] border-b border-transparent focus:border-emerald-500/30 pb-0.5 transition-all"
                  placeholder="e.g. 07XXXXXXXX"
                />
              </div>
            </div>

            {/* Context-Driven Inline Error Handling */}
            {phoneError && (
              <p className="text-[10px] text-rose-400 font-medium text-right animate-in fade-in duration-200">
                {phoneError}
              </p>
            )}
            ...
            <div className="flex justify-between text-xs pt-2 border-t border-zinc-800">
              ...
              <span className="font-mono font-black text-white">KES {billTotal}</span>
            </div>
          </div>

          {/* State Conditional Render Tree */}
          {mpesaStatus === 'idle' && (
            <button 
              onClick={executeSTKPush}
              disabled={!!phoneError || !mpesaPhone}
              className="w-full py-2.5 bg-emerald-500 hover:bg-emerald-400 disabled:bg-zinc-800 disabled:text-zinc-500 disabled:cursor-not-allowed text-zinc-950 font-black text-xs rounded-xl transition-all active:scale-[0.99]"
            >
              Initiate Pay Push
            </button>
          )}
          ...
          {mpesaStatus === 'pin_prompted' && (
            <div className="space-y-4 py-1">
              <div className="p-3 bg-zinc-900 border border-amber-500/20 rounded-xl text-xs text-amber-300/90 flex items-start gap-2 animate-in fade-in zoom-in-95">
              ...
              <button 
                ...
                className="w-full py-2.5 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 text-xs font-black rounded-xl transition-all active:scale-[0.99]"
              >
          ...
          {mpesaStatus === 'success' && (
            <div className="py-4 text-center space-y-2.5 animate-in zoom-in-95 duration-200">
              <div className="w-10 h-10 bg-emerald-500 text-zinc-950 rounded-full flex items-center justify-center mx-auto shadow-lg shadow-emerald-500/10">
