...
import { 
  ...
  ChevronRight, Clipboard, LogOut, ChevronDown, CheckCircle2
} from 'lucide-react';
...
export function UserProfile({ onBackToChat }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-350/50  flex items-center justify-center p-0 animate-in fade-in duration-200">
      <div className="w-screen h-screen bg-slate-50 text-slate-800 font-sans flex flex-col md:flex-row relative overflow-hidden">
        
        {/* MOBILE HEADER */}
        <div className="h-16 px-4 bg-white border-b border-slate-200/80 flex items-center justify-between md:hidden shrink-0">
        ...
        <div className={`bg-gray-350/70  border-r border-[#3C3147]  text-slate-700 flex flex-col justify-between shrink-0 border-r border-slate-200/80 transition-all duration-300 ease-in-out ${isNavMinimized ? 'md:w-20' : 'md:w-64'} hidden md:flex`}>
          <div>
            {/* Header */}
            <div className={`h-16 ${isNavMinimized ? 'px-3 justify-center' : 'px-6 justify-between'} flex items-center border-b border-slate-100`}>
              <div className="flex items-center gap-3 overflow-hidden">
                {!isNavMinimized && (
                  <div className="p-2 bg-[#3C3147]  border border-indigo-100 rounded-full shrink-0">
                    <User size={18} color={"white"} className="text-indigo-60" />
                  </div>
                )}
                {!isNavMinimized && (
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center  rounded-3xl  ${
                      isNavMinimized ? 'justify-center py-3 px-0' : 'justify-between px-3.5 py-3'
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Icon size={18} className={isActive ? 'text-indigo-60 shrink-0' : 'text-slate-400 shrink-0'} />
          ...
          <div className="p-3 border-t border-slate-100 space-y-1.5">
            <button
              ...
              className={`w-full flex items-center ${
                isNavMinimized ? 'justify-center py-3' : 'justify-start gap-3 px-3.5 py-3'
        ...
        {/* MAIN CONTENT AREA */}
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-slate-50/60 w-full">
          
          {/* HORIZONTAL TAB NAVIGATION (Mobile & Responsive Bar) */}
          <div className="bg-gray-350/50  px-4 md:px-8 border-b border-slate-200/80 flex items-center gap-2 md:gap-6 overflow-x-auto shrink-0 scrollbar-none">
            {tabs.map((tab) => {
              const isActive = activeTab === tab.id;
              const Icon = tab.icon;
              return (
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-8 w-full">
            <div className={`w-full grid  ${ShowPersonalDetails ? ' grid-cols-1 lg:grid-cols-12 gap-6 items-star ' : ' grid-cols-1 items-center' }`}>
              
              {/* LEFT COLUMN: Profile Overview (Only on Identity Tab) */}
              {activeTab === 'identity' && (
                <div className={`lg:col-span-4 bg-[whitecloud]  rounded-md  border border-slate-200/80 p-6 shadow-sm space-y-6 ${ShowPersonalDetails ? '':'hidden'} `}>
                  ...
                  <div className="space-y-3 pb-6 border-b border-slate-100">
                    ...
                    <div className="space-y-2.5 text-xs">
                      <div className="flex items-center gap-3 text-slate-600">
                        ...
                        <span className="font-mono text-slate-800 font-medium">{profile.phone || 'No phone set'}</span>
                      </div>
                      <div className="flex items-center gap-3 text-slate-600">
                        <Mail size={15} className="text-slate-400 shrink-0" />
                        <span className="truncate text-slate-800 font-medium">{profile.email || 'No email set'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Address Summary */}
                  <div className="space-y-3 pb-6 border-b border-slate-100">
                    <h4 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Default Delivery</h4>
                  ...
                  <div className="space-y-3">
                    <h4 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Details</h4>
                    <div className="space-y-2.5 text-xs">
                      ...
                      <div className="flex items-center justify-between py-0.5">
                        ...
                        <span className="font-semibold font-mono text-slate-800">{profile.national_id_number || '—'}</span>
                      </div>
                      <div className="flex items-center justify-between py-0.5">
                        <span className="text-slate-500 flex items-center gap-2">
              ...
              <div className={`space-y-6  bg-[whitecloud]  rounded-md  ${activeTab === 'identity' ? 'lg:col-span-8 ' : 'lg:col-span-12'}`}>
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    {activeTab === 'identity' && (
                      <motion.div 
                        ...
                        className="bg-[whitecloud] rounded-md border border-slate-200/80 p-6 md:p-8 shadow-sm space-y-6"
                      >
                        <div className="flex items-center justify-between pb-5 border-b border-slate-100">
                          <div>
                            <h3 className="text-base font-bold text-slate-900">Personal Information</h3>
                            <p className="text-xs text-slate-500 mt-0.5">Manage your legal identity and contact credentials</p>
                          </div>
                        </div>
                        ...
                        <CleanInput label="Email Address" type="email" icon={<Mail size={16} />} value={profile.email} disabled={!!user?.email} onChange={(v) => handleFieldChange('email', v)} />

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <CleanInput label="Phone Number" disabled={!!user?.phone} value={profile.phone} icon={<Phone size={16} />} onChange={(v) => handleFieldChange('phone', v)} />
                          <CleanInput label="National ID / Passport" disabled={!!user?.national_id_number} value={profile.national_id_number} icon={<Hash size={16} />} onChange={(v) => handleFieldChange('national_id_number', v)} />
                        </div>
                      ...
                      >
                        <div className="flex items-center justify-between pb-5 border-b border-slate-100">
                          <div>
                            ...
                            <p className="text-xs text-slate-500 mt-0.5">Set default address details and delivery directions</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-full flex flex-col gap-1.5 bg-slate-50 p-3.5 rounded-2xl border border-slate-200/80 focus-within:border-indigo-600 focus-within:bg-white focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">County</label>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-full flex flex-col gap-1.5 bg-slate-50 p-3.5 rounded-2xl border border-slate-200/80 focus-within:border-indigo-600 focus-within:bg-white focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Sub-County</label>
                            <div className="relative flex items-center">
                              <select 
                    ...
                    {activeTab === 'cart' && (
                      ...
                      >
                        <div className="flex items-center justify-between pb-5 border-b border-slate-100">
                          <div>
                            <h3 className="text-base font-bold text-slate-900">Active Cart Items</h3>
                        ...
                        {cart?.length > 0 ? (
                          <div className="space-y-6">
                            <div className="divide-y divide-slate-100">
                              {cart.map((item) => (
                                <div key={item.product.id || item.id} className="py-4 flex items-center justify-between gap-4">
                                  <div>
                                    <p className="text-xs font-bold text-slate-900">{item.product.name}</p>
                                    <p className="text-xs font-mono font-medium text-slate-500 mt-0.5">KES {item.product.price?.toLocaleString()}</p>
                                  </div>
                            ...
                            <div className="pt-4 space-y-3 bg-slate-50/80 p-5 rounded-2xl border border-slate-200/60">
                              <div className="flex justify-between text-xs text-slate-500">
                                <span>Subtotal</span>
                                <span className="font-mono text-slate-900 font-semibold">KES {cartSummary?.subtotal?.toLocaleString() ?? 0}</span>
                              </div>
                              <div className="flex justify-between items-center font-bold text-xs text-slate-900 pt-3 border-t border-slate-200/60">
                                <span>Total Amount</span>
                                <span className="font-mono text-indigo-600 text-base">KES {cartSummary?.finalTotal?.toLocaleString() ?? 0}</span>
                              </div>
                            </div>
                          </div>
                        ) : (
                    ...
                    {activeTab === 'orders' && (
                      ...
                      >
                        <div className="flex items-center justify-between pb-5 border-b border-slate-100">
                          <div>
                            <h3 className="text-base font-bold text-slate-900">Order Activity</h3>
                            <p className="text-xs text-slate-500 mt-0.5">Historical log of all completed transactions</p>
                          </div>
                        </div>
                        <div className="mt-4">
          ...
          <AnimatePresence>
            {(isModified || saveSuccess) && ['identity', 'logistics'].includes(activeTab) && (
              <motion.div 
              ...
              >
                {saveSuccess ? (
                  ...
                  <>
                    <span className="text-xs font-medium text-slate-300 px-2">Unsaved changes detected</span>
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
