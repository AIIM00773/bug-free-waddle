...
export function UserCart({ onBackToChat }) {
  ...
  const tabs = [
    ...
    { id: 'saved', label: 'Saved for Later', icon: ShoppingBag, count: 0 }
  ];
      ...
      <div className="w-full h-full md:max-w-full md:h-[100vh] bg-[#F8FAFC] text-slate-800 font-sans flex flex-col md:flex-row relative rounded-none shadow-2xl overflow-hidden">
        ...
        <div 
          className={`bg-[#191a1a]  text-slate-300 flex flex-col justify-between shrink-0 border-r border-slate-800 transition-all duration-300 ease-in-out ${
            isNavMinimized ? 'md:w-15' : 'md:w-54'
          } w-full`}
        >
          <div>
            {/* Header with Toggle */}
        ...
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-[#F8FAFC]">
          ...
          <div className="h-16 px-8 bg-[#191a1a]  border-b border-slate-200/80 flex items-center justify-between shrink-0">
            <div>
              <h1 className="text-lg font-semibold text-slate-50">Shopping Cart</h1>
            </div>
            ...
            <div className="max-w-6xl mx-auto">
              ...
              {orderPlaced ? (
                <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-2xl border border-slate-200/80 p-8 space-y-4">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-base font-semibold text-slate-900">Order Dispatched Successfully!</h3>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                  ...
                  <div className="lg:col-span-8 space-y-4">
                    {cartItems.length > 0 ? (
                          ...
                          >
                            <div className="flex items-start gap-4">
                              ...
                              <div className="space-y-1">
                                ...
                                <h4 className="font-medium text-xs text-slate-900 pt-1">{item.name}</h4>
                                <p className="font-mono font-semibold text-xs text-slate-700">
                                  KES {item.price.toLocaleString()} <span className="text-[10px] text-slate-400 font-normal">per unit</span>
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between sm:justify-end gap-6 pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-100">
                              <div className="flex items-center gap-2 bg-slate-50 border border-slate-200/80 rounded-xl p-1">
                                ...
                                <span className="font-mono text-xs font-semibold px-2 text-slate-800">{item.qty}</span>
                                <button
                                  type="button"
                                  onClick={() => updateQty(item.id, 1)}
                                  className="p-1 rounded-lg text-slate-500 hover:bg-white hover:text-slate-800 transition-colors"
                      ...
                      <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-3 bg-white rounded-2xl border border-slate-200/80">
                        ...
                        <p className="text-xs font-medium text-slate-500">Your cart is currently empty</p>
                        <button
                          type="button"
                          onClick={onBackToChat}
                  ...
                  <div className="lg:col-span-4 bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm space-y-6">
                    <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
                      ...
                      <div>
                        <h3 className="font-semibold text-slate-900 text-xs uppercase tracking-wider">Order Summary</h3>
                        <p className="text-[11px] text-slate-500">Micro-logistics & Dispatch</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3 text-xs pb-4 border-b border-slate-100">
                      <div className="flex justify-between text-slate-600">
                        ...
                        <span className="font-mono font-medium text-slate-900">KES {subtotal.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-slate-600">
                        <span>Runner Delivery Fee</span>
                        <span className="font-mono font-medium text-slate-900">KES {deliveryFee.toLocaleString()}</span>
                      </div>
                    ...
                    <div className="flex justify-between items-center pt-2">
                      <span className="text-xs font-semibold text-slate-900 uppercase tracking-wider">Total</span>
                      <span className="font-mono font-bold text-base text-slate-900">KES {total.toLocaleString()}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-center gap-1.5 text-[10px] text-slate-400 pt-2">
                      <ShieldCheck size={14} className="text-emerald-600" /> Secure local vendor transaction
                    </div>
                  </div>
